package coms309.TriviaBum.Websockets.Score;
...
public class ScoreSocket {
    ...
    // server side logger
    private final Logger logger = LoggerFactory.getLogger(ScoreSocket.class);
    ...
    public void onMessage(Session session, String message) throws IOException {
        ...
        double seconds = Math.round(Integer.parseInt(message) / 1000.0);

        double score = seconds / 30.0;

        int finalscore = (int) Math.round(score * 100);

        logger.info("[SCORE] " + username + ": " + finalscore);

        session.getBasicRemote().sendText(String.valueOf(finalscore));
